...
import cont_assoc.models.dsu4d_models as models
...
def main(config_ps, config_u, ckpt_ps, ckpt_u, save, test_set):
    ...
    model = models.DSU4D(ps_cfg, u_cfg)
...
def create_dirs(test_set):
    results_dir = join(getDir(__file__), 'output', 'val', 'dsu4d', 'sequences')
    if test_set:
        results_dir = join(getDir(__file__), 'output', 'test', 'dsu4d', 'sequences')
